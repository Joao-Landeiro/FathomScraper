...
Version 2 with enhanced participant structure support.
"""
import asyncio
import json
import logging
...
class FathomScraper:
    ...
    def _parse_meeting_data_from_api_v2(self, api_item: Dict) -> Dict:
        """
        Parses a single meeting item from the API response into our enhanced format.
        ...
        Returns:
            Dict containing parsed meeting data with enhanced participant structure
        """
        self.logger.info(f"Parsing meeting data with enhanced participant structure for meeting ID: {api_item.get('id')}")
    ...
    def _validate_participants(self, participants: List[Dict]) -> None:
        ...
        """
        self.logger.debug("Validating participant data structure")
        ...
        for participant in participants:
            ...
            # Validate participant_id (optional but should be string if present)
            if 'participant_id' in participant and not isinstance(participant['participant_id'], str):
                self.logger.warning(f"Invalid participant_id type: {participant}")
